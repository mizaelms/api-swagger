#!/bin/bash
	...
	for PRO in $( oc get projects -o name);
        do
		    ...
		    if [[ ${PA} != *"openshift"* ]];then
				...
				if [[ ${LINK} = *"http"* ]];then
					echo "LINK: $LINK"
					echo "<tr><td>$PA</td><td><a href='api-doc/$P/index.html' target='_blank'>$P</a></td><td><a href='http://swagger-editor-api-documentation-uat.oscpuat.gnp.com.mx/?url=$P.yaml' target='_blank'>$P.json</a></td><td>$DESC<div id='tags' style='display:none'>$TAGS $SUMMARY</div></td></tr>" >> index.html
					curl -o $P.zip $LINK
					unzip $P.zip
	...
	mv *.json json/
	mv *.yaml yaml/
	oc project api-documentation-uat
