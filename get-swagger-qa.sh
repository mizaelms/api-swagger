#!/bin/bash
	...
	do
        ...
        if [[ ${LS} = *"<title>Swagger UI</title>"* ]];then
            ...
            echo $DESC
            echo "$JSON" >> api-$counter.json
            sed -i '' 's/,"GNP":"Grupo Nacional Provincial"//g' *.json
            ruby -ryaml -rjson -e 'puts YAML.dump(JSON.parse(STDIN.read))' < api-$counter.json > api-$counter.yaml
			...
			if [[ ${LINK} = *"http"* ]];then
					echo "LINK: $LINK"
					echo "<tr><td>Cloud</td><td><a href='api-doc/api-$counter/index.html' target='_blank'>$DESC</a></td><td><a href='http://swagger-editor-api-documentation-qa.oscp.gnp.com.mx/?url=api-$counter.yaml' target='_blank'>$DESC</a><div id='tags' style='display:none'>$TAGS $SUMMARY</div></td><td></td></tr>" >> cloud.html
					curl -o api-$counter.zip $LINK
					unzip api-$counter.zip
					mv html2-client api-doc/api-$counter
					rm api-$counter.zip
			fi;
	...
	echoo "</tbody></table></body>" >> cloud.html	
	...
	for PRO in $( oc get projects -o name);
        do
		    ...
		    if [[ ${PA} != *"openshift"* ]];then
				...
				if [[ ${LINK} = *"http"* ]];then
					echo "LINK: $LINK"
					echo "<tr><td>$PA</td><td><a href='api-doc/$P/index.html' target='_blank'>$P</a></td><td><a href='http://swagger-editor-api-documentation-qa.oscp.gnp.com.mx/?url=$P.yaml' target='_blank'>$P.json</a></td><td>$DESC<div id='tags' style='display:none'>$TAGS $SUMMARY</div></td></tr>" >> index.html
					curl -o $P.zip $LINK
					unzip $P.zip
	...
	mv *.json json/
	mv *.yaml yaml/
	oc project api-documentation-qa
    ...
    sed -i '' 's/info|description|//g' cloud.html
    cp index.html /Users/mizael/Documents/workspace-spring-tool-suite-4-4.3.2.RELEASE/api-swagger/
    cp cloud.html /Users/mizael/Documents/workspace-spring-tool-suite-4-4.3.2.RELEASE/api-swagger/
